...
4. **第二轮迭代（凭据复用 / REST 向量）**：发现 `/user/login?_format=json` REST 登录端点可用（返回结构化错误）；批量尝试跨题凭据复用（admin/admin 与 4 组跨题口令）全部 400「unrecognized username or password」；6 次失败后触发 Drupal flood control（403「Too many failed login attempts from your IP」）。**第三轮（flood 过期后走表单通道）**：带 form_build_id 的表单登录再复用 7 组跨题凭据（共 4 组跨题口令）全部 200 且无跳转 = 均未命中；`hal_json` REST 向量因站点无 node 且 rest/hal 未路由（/node/1 404）不可达。
